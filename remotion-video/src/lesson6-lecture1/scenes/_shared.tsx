...
export const IMG = (file: string) => staticFile(`lesson6-lecture1/images/${file}`);
...
export const PromptQuote: React.FC<{
  ...
  >
    <span style={{ color, fontSize: 30, fontWeight: 900, lineHeight: 1.4, flexShrink: 0 }}>❝</span>
    <span>{text}</span>
  </div>
);
    ...
    <SceneShell accent={accent} variant="flow" bg={bg}>
      ...
      >
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-start", gap: 4 }}>
          <span style={{ color: accent, fontSize: 27, fontWeight: 800, direction: "rtl" }}>{`כיוון ${index}`}</span>
          <span style={{ color: COLORS.text, fontSize: 64, fontWeight: 900, textShadow: `0 0 30px ${accent}44` }}>{title}</span>
        </div>
      </div>
      ...
      >
        <div style={{ maxWidth: 1300, textAlign: "center", direction: "rtl", color: COLORS.textMuted, fontSize: 36, fontWeight: 600, lineHeight: 1.4 }}>
          {when}
        </div>
      </div>
      ...
      {footer && (
        <div style={{ position: "absolute", bottom: 70, left: 0, right: 0, display: "flex", justifyContent: "center", opacity: footerS, transform: `translateY(${interpolate(footerS, [0, 1], [16, 0])}px)` }}>
          <div style={{ maxWidth: 1320, textAlign: "center", direction: "rtl", fontSize: 32, fontWeight: 800, color: COLORS.text }}>{footer}</div>
        </div>
      )}
    </SceneShell>
  );
