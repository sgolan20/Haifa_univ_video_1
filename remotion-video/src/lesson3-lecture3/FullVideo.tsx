...
import { Shot10_2 } from "./scenes/Shot10_2";
 ...
 * Lip-synced (VEED Fabric 1.0, 720p) to the first sentences of the narration —
 * "שלום לכולם. יושרה אקדמית אינה מסתיימת בשימוש נכון," (cut at 4.79s silence gap).
 * The clip's own audio is muted — the synced full_narration.mp3 is the source.
 */
const INTRO_END_FRAME = 144; // 4.79s @ 30fps (matches the generated clip length)
...
const TitleCardDissolve: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ opacity }}>
      <TitleCard
        title="שקיפות ותיעוד — ציטוט AI ויצירת Appendix"
...
const IntroOverlay: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ opacity }}>
      <OffthreadVideo
        src={staticFile("lesson3-lecture3/video/intro_talking_head.mp4")}
        muted
...
/**
 * FullVideo — Master composition for Lesson 3 / Lecture 3
 * "שקיפות ותיעוד — ציטוט AI ויצירת Appendix"
 */
  ...
  return (
    <AbsoluteFill>
      ...
      <Sequence from={TITLE_CARD_FRAMES}>
        <Audio src={staticFile("lesson3-lecture3/audio/full_narration.mp3")} volume={1} />
      </Sequence>
