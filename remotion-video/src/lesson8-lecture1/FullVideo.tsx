...
const SHOT_COMPONENTS: Record<string, React.FC> = {
  "shot1-1": Shot1_1, "shot2-1": Shot2_1, "shot3-1": Shot3_1, "shot4-1": Shot4_1,
  "shot5-1": Shot5_1, "shot6-1": Shot6_1, "shot7-1": Shot7_1, "shot8-1": Shot8_1,
  "shot9-1": Shot9_1, "shot10-1": Shot10_1, "shot11-1": Shot11_1, "shot12-1": Shot12_1,
  "shot13-1": Shot13_1, "shot14-1": Shot14_1, "shot15-1": Shot15_1, "shot16-1": ClosingLogo,
};
...
const INTRO_END_FRAME = 336; // 11.2s @ 30fps — fabric-1.0 lip-sync clip length

const IntroOverlay: React.FC = () => {
  const frame = useCurrentFrame();
  const opacity = interpolate(frame, [INTRO_END_FRAME - 12, INTRO_END_FRAME], [1, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  return (
    <AbsoluteFill style={{ opacity }}>
      <OffthreadVideo src={staticFile("lesson8-lecture1/video/intro_talking_head.mp4")} muted style={{ width: "100%", height: "100%", objectFit: "cover" }} />
    </AbsoluteFill>
  );
};
...
const TitleCardDissolve: React.FC = () => {
  const frame = useCurrentFrame();
  const opacity = interpolate(frame, [TITLE_CARD_FRAMES, TITLE_CARD_FRAMES + TITLE_TO_VIDEO_DISSOLVE_FRAMES], [1, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  return (
    <AbsoluteFill style={{ opacity }}>
      <TitleCard title="עריכה ושכתוב בעזרת AI — תוך שמירה על קול אישי" withVisualFadeOut={false} />
    </AbsoluteFill>
  );
};

export const FullVideoL8L1: React.FC = () => {
  const frame = useCurrentFrame();
  let cumulativeFrame = 0;
  const logoOpacity = interpolate(frame, [LAST_SHOT_START - 30, LAST_SHOT_START], [1, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });

  return (
    <AbsoluteFill>
      <Sequence from={TITLE_CARD_FRAMES}>
        <Audio src={staticFile("lesson8-lecture1/audio/full_narration.mp3")} volume={1} />
      </Sequence>
